...
public class GenreViewHolder extends GroupViewHolder {
  ...
  public GenreViewHolder(View itemView) {
    ...
    headerFL = (FrameLayout)itemView.findViewById(R.id.headerFL);
  }

  public void setGenreTitle(Context mContext,ExpandableGroup genre) {
    if (genre instanceof Genre) {
      genreName.setText(genre.getTitle());
      icon.setBackgroundResource(((Genre) genre).getIconResId());
      String themeArra[] = Utils.convertString2Array(mContext);
      headerFL.setBackgroundColor(Color.parseColor(themeArra[16]));
      genreName.setTextColor(Color.parseColor(themeArra[28]));
    }
  }
